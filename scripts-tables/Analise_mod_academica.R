path1 <- sprintf("tables/%s/tab1-mod_academica-%s.png",year,year) 
path2 <- sprintf("tables/%s/tab2-mod_academica-%s.png",year,year) 
...
micro_cursos$TP_MODALIDADE_ENSINO <- factor(micro_cursos$TP_MODALIDADE_ENSINO)
...
tab1 <- micro_cursos %>%
  drop_na() %>%
  crosstable(NO_CINE_AREA_DETALHADA, by = TP_MODALIDADE_ENSINO, label = T,
...
tab2 <- micro_cursos %>%
  drop_na() %>%
  crosstable(NO_CINE_ROTULO, by = TP_MODALIDADE_ENSINO, label = T,
             total = "both", percent_pattern = "{n} ({p_col}) ({p_row})",
  ) %>%
  as_flextable(keep_id = F, compact = T, autofit = F,
               fontsizes = list(body = 10, subheaders = 10, header = 10)
