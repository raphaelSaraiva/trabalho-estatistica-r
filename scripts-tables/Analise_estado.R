path1 <- sprintf("tables/%s/tab1-estado-%s.png",year,year) 
path2 <- sprintf("tables/%s/tab2-estado-%s.png",year,year) 
...
micro_cursos$NO_UF <- factor(micro_cursos$NO_UF)
...
tab1 <- micro_cursos %>%
  drop_na() %>%
  crosstable(NO_UF, by = NO_CINE_AREA_DETALHADA, label = T,
...
tab2 <- micro_cursos %>%
  drop_na() %>%
  crosstable(NO_UF, by = NO_CINE_ROTULO, label = T,
             total = "both", percent_pattern = "{n} ({p_col}) ({p_row})",
  ) %>%
  as_flextable(keep_id = F, compact = T, autofit = F,
               fontsizes = list(body = 10, subheaders = 10, header = 10)
